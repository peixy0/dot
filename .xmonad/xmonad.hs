...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
  ...
  -- launch app menu
  , ((modm, xK_p), spawn "rofi -combi-modi run,ssh -show combi")
...
myLayout = spacingRaw False (Border 10 0 10 0) True (Border 0 10 0 10) True $
  tile ||| tileMirrored
  where
    tile = mouseResizableTile{ draggerType = dragger }
    tileMirrored = mouseResizableTileMirrored{ draggerType = dragger }
    dragger = FixedDragger 0 20
